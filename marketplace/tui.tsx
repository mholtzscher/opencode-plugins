...
export default Plugin.define({
  ...
  setup(context) {
    ...
    const removePanel = context.ui.slot({
      ...
      render: (panel) => (
        <Show when={panel.name === "marketplace.installed"}>
          <MarketplaceInstalledPanel marketplace={marketplace} panel={panel} />
        </Show>
      ),
    });
      ...
      render: () => {
        context.keymap.layer(() => ({
          commands: [
            {
              group: "Marketplace",
              id: "marketplace.close",
              palette: true,
              run: closeCatalog,
              title: "Close marketplace catalog",
            },
            {
              group: "Marketplace",
              id: "marketplace.open",
              ...
              run: () => {
                if (!context.ui.panel.open("marketplace.installed")) {
                  context.ui.toast.show({
                    message: "Open a session to use the installed-items panel",
                    variant: "info",
                  });
                }
              },
              title: "Manage installed marketplace items",
            },
          ],
          mode: "global",
        }));
